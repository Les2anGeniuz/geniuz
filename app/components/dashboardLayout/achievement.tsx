...
const Achievements: React.FC = () => {
  ...
  // UBAH 1: Loading fix 395px
  if (loading) return (
    <div className="w-full h-[395px] bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex items-center justify-center">
  ...
  return (
    // UBAH 2: Fix Height h-[395px] (Bukan min-h)
    <div className="w-full h-[395px] bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex flex-col">
      <h2 className="text-xl font-bold text-black mb-4 flex-shrink-0">Pencapaian</h2>
      
      {/* UBAH 3: Scrollable Area */}
      <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar">
        {hasData ? (
            ...
            {achievements.map((item) => {
              ...
              return (
                <div key={item.id} className="flex items-center gap-4 bg-gray-50 p-3 rounded-lg border border-gray-100">
                  <div className={`w-10 h-10 flex-shrink-0 flex items-center justify-center rounded-lg ${style.color}`}>
                    {style.icon}
                  </div>
          ...
          <div className="flex flex-col items-center justify-center h-full text-center pb-4">
             ...
             <h3 className="text-gray-900 font-semibold text-lg">Belum Ada Pencapaian</h3>
             <p className="text-gray-500 text-sm mt-2 max-w-[250px]">
               Yuk, tetap semangat belajar dan kerjakan tugasmu untuk membuka achievement baru!
             </p>
          </div>
        )}
      </div>
