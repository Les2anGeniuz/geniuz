...
export default function PengaturanPage() {
  ...
  // Mengambil Base URL dari env seperti di Sidebar kamu
  const API_BASE = useMemo(() => process.env.NEXT_PUBLIC_API_BASE_URL ?? "http://localhost:5000/api", []);

  // 1. Logika Fetch Data Profile (Referensi dari Sidebar.tsx)
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // Memanggil endpoint /me/profile yang baru kita buat di backend
  ...
  const handleUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      const fileName = `avatar-${userData.id_User}-${Date.now()}.${fileExt}`;
      ...
      // Update URL ke database lewat API backend
        ...
        <main className="p-8 pt-24 max-w-6xl mx-auto">
          <div className="grid grid-cols-12 gap-8">
            ...
            <div className="col-span-4 bg-white p-8 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center">
              ...
              >
                {/* Menggunakan placeholder jika foto_profil null */}
                <Image 
                  src={userData.foto_profil || '/placeholder-user.jpg'} 
                  alt="Avatar" fill className="object-cover"
                />
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all text-white text-[10px] font-bold uppercase">
              ...
              <div className="text-center mt-1">
                {/* Data dinamis dari tabel Pendaftaran */}
            ...
            <div className="col-span-8 bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-2 col-span-2">
                  ...
                  <input type="email" defaultValue={userData.email || ''} className="w-full border border-gray-200 p-3 rounded-xl focus:ring-2 focus:ring-[#064479] outline-none transition-all bg-white font-medium" />
                </div>
                <div className="space-y-2 col-span-2">
                ...
                <button className="col-span-2 mt-4 bg-[#064479] text-white font-bold py-3 rounded-xl hover:bg-[#05355d] transition-all shadow-lg shadow-blue-900/10">
                  Simpan Perubahan
                </button>
              </div>
            </div>
