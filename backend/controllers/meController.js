...
export const getMyKelas = async (req, res) => {
  try {
    ...
    if (!pendaftarans || !pendaftarans.length) {
    ...
    if (mentorError) {
      return res.status(500).json({ error: mentorError.message })
    }
    ...
    ;(fakultasRows || []).forEach(f => {
      fakultasMap.set(f.id_Fakultas, f)
    })

    const mentorMap = new Map()
    ;(mentorRows || []).forEach(m => {
      mentorMap.set(m.id_Mentor, m)
    })
